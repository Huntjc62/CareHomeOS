...
async function createRecord(col,payload,message){
 try{await addDoc(collection(db,`organisations/${state.org.id}/${col}`),{...payload,createdBy:state.user.uid,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});closeModal();toast(message)}catch(e){toast(prettyError(e))}
}
async function editRecord(col,id,payload,message){
 try{await updateDoc(doc(db,`organisations/${state.org.id}/${col}/${id}`),{...payload,updatedBy:state.user.uid,updatedAt:serverTimestamp()});closeModal();toast(message)}catch(e){toast(prettyError(e))}
}
...
audit:()=>pageHead("Audit history","Account-owner change history. Entries are created by the Firebase backend when records change.",can("audit")?btn("Refresh","go('audit')"):"")+`<div class="card table-card"><div class="table-tools"><input class="search" placeholder="Search audit history..." oninput="filterRows(this.value)"></div>${table(["Date","User","Action","Area","Record",""],(state.audit||[]).map(a=>`<tr><td>${fmtDate(a.at)}</td><td>${esc(a.userName||a.uid)}</td><td>${esc(a.action)}</td><td>${esc(a.collection)}</td><td>${esc(a.recordLabel||a.recordId)}</td><td><span class="audit">${esc(a.summary||"")}</span></td></tr>`))}</div>`,
...
if(!configured){
 document.body.innerHTML=`<div class="login"><div class="login-box"><div class="login-brand"><div class="brand-mark">C</div><div><h1>CareHomeOS</h1><small>Firebase setup required</small></div></div><p>Open <b>firebase-config.js</b> and paste the Firebase Web App configuration supplied by the Firebase console.</p><div class="notice info">The package is intentionally shipped without your Firebase credentials.</div><p class="audit">After configuring Firebase, enable Email/Password Authentication, create a Firestore database, deploy the supplied Security Rules and deploy the optional Cloud Functions.</p></div></div>`;
}else{
